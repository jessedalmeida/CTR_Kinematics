...
namespace CTR
{
   ...
   namespace internal
   {
      ...
      typename Vector<3>::type xi_n( const Context &c )
      {
         ...
         return Functions::zero_fun< typename Vector<3>::type >()();	// This is hard-coding n' = 0 (distributed load would normally be n'=-f)
		 // Need to look carefully at these equations and make sure these 
		 // Need to find where the derivative of this equation is done
		 // All the Jacobians are in xi_fun_impl_Z... 
		 // Test it against a known case (i.e. a straight beam cantilevered)
		 // 
      }
